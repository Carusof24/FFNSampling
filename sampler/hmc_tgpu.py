...
from utils.operations import  compute_d, compute_d2, compute_mod2, compute_mod, compute_q,  wcopy, load_torch_state, save_torch_state, init_torch_generator
# Default precision
...
class HMCSampler():
    ...
    def __init__(self, model, w0, Cost, Metric, dataset, generator, name:str='HMCSampler'):
        self.model = model
        self.w0 = w0
        self.Cost = Cost
        self.Metric = Metric
    ...
    def sample(
        ...
        # ——— all’inizio del file, se vuoi essere esplicito ———
        assert torch.cuda.is_available(), "Serve una GPU CUDA"

        for _ in range(self.pars['moves']):
            wi = self.model.copy(grad=False)

            # ---- setup eventi e stream ----
            ...
            e_int_s  = torch.cuda.Event(enable_timing=True)
            e_int_e  = torch.cuda.Event(enable_timing=True)
            e_acc_s  = torch.cuda.Event(enable_timing=True)
            e_acc_e  = torch.cuda.Event(enable_timing=True)
            ...
            with torch.cuda.stream(stream):
                ...
                # ---- integrazione ----
                e_int_s.record(stream)
                self._init_momenta()
                Ki   = self._compute_K()
                obf  = self._integrate()
                Kf   = self._compute_K()
                e_int_e.record(stream)

                # ---- accettazione ----
                e_acc_s.record(stream)
                dE = (obf['loss'] - self.data['loss'] + Kf - Ki).item()

                p = torch.rand(1, device=self.model.device,
                ...
                e_acc_e.record(stream)

                # ---- statistiche ----
                self.check['move'] += 1
                self.check['d2']        = compute_d2(self.model.weights, self.w0).item()
                self.check['similarity'] = compute_q(self.model.weights, self.w0).item()

                # ---- fine mossa ----
                e_move_e.record(stream)

            # aspetta la fine dello stream prima di leggere i tempi
            stream.synchronize()

            # ms → s
            t_int = e_int_s.elapsed_time(e_int_e) / 1000.0
            t_acc = e_acc_s.elapsed_time(e_acc_e) / 1000.0
            tpc   = e_move_s.elapsed_time(e_move_e) / 1000.0

            # registra
            self.check['t_int'] = t_int
            self.check['t_acc'] = t_acc
            self.check['tpc']   = tpc
            self.check['d2'] = compute_d2(self.model.weights, self.w0).item()
            self.check['similarity'] = compute_q(self.model.weights, self.w0).item()
            self.data['move'] += 1

            self.check['eff_D'] = self.check['d2'] / (tpc * self.data['move'])
            self.check['eff_v'] = self.check['btb_d'] / tpc


            # housekeeping invariato (save_step, print_step, ecc.)
    ...
    # Accesso corretto ai tensori del dataset
