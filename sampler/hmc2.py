...
# --- integrazione leap-frog —
 ...
 # --- Metropolis + logging ———
            ...
            acc_dt = ptime() - acc_start 

            self.check['lf_dt']   = lf_dt
            self.check['acc_dt']  = acc_dt
            self.data['move'] += 1
            self.data['time'] = ptime()-self.t0
            self.check['move'] += 1
            self.check['d2'] = compute_d2(self.model.weights, self.w0).item()
            self.check['eff_D'] = self.check['d2']/ (self.pars['isteps'] * self.data['move'])
            self.check['step_dt'] = ptime()-step_dt
            self.check['eff_v'] = self.check['btb_d']/self.pars['isteps']
