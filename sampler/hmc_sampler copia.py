...
# ---- accettazione ----
            ...
            self.check[ 'tpc' ] = ptime() - t_int0
            self.data['tpc_sum'] += tpc
            self.data['tpc_avg']  = self.data['tpc_sum'] / self.data['move']

            
            self.check['move'] += 1
            self.check['d2'] = compute_d2(self.model.weights, self.w0).item()
            self.check['eff_D'] = self.check['d2']/ (self.check['tpc'] * self.data['move'])
            
            self.check['eff_v'] = self.check['btb_d']/self.check['tpc']
